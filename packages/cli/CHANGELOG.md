...
- a197f32: `factlas extract` now prints **NDJSON by default** — one fact per line — so the output loads straight into a database (querying facts for evaluation is the whole point of the stream). Each line is a complete, content-addressed fact record; pipe it into SQLite/DuckDB/`jq` with no unwrapping.
...
- Updated dependencies [d486b66]
  - @factlas/core@0.4.0
  - @factlas/plugin-css@4.0.0
  - @factlas/plugin-inline-style@4.0.0
  - @factlas/plugin-jsx@4.0.0
  - @factlas/plugin-styled@4.0.0
  - @factlas/plugin-tailwind@4.0.0
